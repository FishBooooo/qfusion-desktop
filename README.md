...
QFusion Desktop 是一个本地优先的美股与港股科技板块“3+1”多视角交易决策平台。项目当前处于 **M0 项目基线**：只提供可验证的 FastAPI 健康检查、React/Tauri 空壳和明确标记的合成 Mock 页面。

> 当前版本不连接真实行情、不运行交易模型、不提供投资建议，也不存在实盘提交路径。
