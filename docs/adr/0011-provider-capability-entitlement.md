...
1. `ProviderCapability` 描述当前 Adapter 实现和官方产品层面的技术能力。市场数据能力
   必须通过 `MarketDataCapability` 按“市场 + 操作”声明周期、实时、盘前盘后和历史起点；
...
调用前必须把请求同时与两份契约按精确“市场 + 操作”键校验。`REALTIME` 只能在同一键下
...
盘前或盘后时段。请求起点必须位于该市场与操作对应、按市场时区解释的
...
延长时段、许可边界和端点权限可以按市场与操作独立审计。新增供应商不能仅因为官方产品
